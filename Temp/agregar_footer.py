...
def generar_html_con_footer(csv_data, output_file):
    """Genera el HTML con footer que incluye la Línea de Reporte"""
...
<body>
    ...
    document.getElementById('unidad-select').addEventListener('change', function() {{
        ...
        // Agregar footer con Línea de Reporte
        const lineaReporte = rows[0]['Línea de Reporte'] || 'No especificado';
        html += `<div class='footer'><strong>Línea de Reporte:</strong> ${{lineaReporte}}</div>`;
...
if __name__ == "__main__":
    try:
        ...
        # Generar HTML con footer
        print("🔨 Generando HTML con footer...")
        generar_html_con_footer(csv_data, "unidades_organizativas_completo.html")
