...
const INPUT = 'organigrama_bna_2025-07-30-3.json';
const OUTPUT = 'organigrama_bna_2025-07-30-3_CORREGIDO.json';
...
function productoFinalConcreto(desc) {
  // Heurística simple: usar la acción principal como producto final
...
function fusionarFunciones(funcs) {
  // Fusiona funciones redundantes (por productoFinal+descripcion normalizados)
  let seen = new Set();
  let out = [];
  for (let f of funcs) {
...
function procesarUnidad(unidad, resumen) {
  ...
  // Redistribuir porcentajes si no suman 100
